...
if (empty($settings['config_sync_directory'])) {
  $settings['config_sync_directory'] = getenv('DRUPAL_CONFIG_SYNC_DIR') ?: dirname($app_root) . '/config/sync';
}
...
$deployment_identifier = getenv('DRUPAL_DEPLOYMENT_IDENTIFIER');
if ($deployment_identifier !== FALSE && $deployment_identifier !== '') {
...
$reverse_proxy = getenv('DRUPAL_REVERSE_PROXY');
if ($reverse_proxy === 'true') {
  $settings['reverse_proxy'] = TRUE;
}

$reverse_proxy_addresses = getenv('DRUPAL_REVERSE_PROXY_ADDRESSES');
if ($reverse_proxy_addresses !== FALSE && trim($reverse_proxy_addresses) !== '') {
  $settings['reverse_proxy_addresses'] = array_values(array_filter(array_map(
    static function (string $address): string {
      return trim($address);
    },
    explode(',', $reverse_proxy_addresses)
  )));
}

$db_driver = getenv('DRUPAL_DB_DRIVER');
if ($db_driver !== FALSE && $db_driver !== '') {
  if (!in_array($db_driver, ['mysql', 'sqlite'], TRUE)) {
    throw new \RuntimeException('Unsupported DRUPAL_DB_DRIVER value.');
  }

  if ($db_driver === 'sqlite') {
    $sqlite_path = getenv('DRUPAL_SQLITE_PATH') ?: dirname($app_root) . '/var/db/drupal.sqlite';
    $databases['default']['default'] = [
      'driver' => 'sqlite',
    ...
    $databases['default']['default'] = [
      ...
      'port' => (string) (getenv('DRUPAL_DB_PORT') ?: '3306'),
      'prefix' => '',
      'collation' => 'utf8mb4_general_ci',
    ];
  }
...
if ($ollama_host !== FALSE && $ollama_host !== '') {
  $config['ai_provider_ollama.settings']['host_name'] = $ollama_host;
}

$ollama_port = getenv('OLLAMA_PORT');
if ($ollama_port !== FALSE && $ollama_port !== '') {
  $config['ai_provider_ollama.settings']['port'] = (int) $ollama_port;
}
